...
import { dayBaseForParisDateStr, parisNow } from "./paris-time";
...
export async function computeVehiclePositions(): Promise<VehiclePosition[]> {
  ...
  const { todayStr, yesterdayStr, nowSec } = parisNow();
  ...
  for (const [tripId, trip] of gtfs.trips) {
    ...
    const dayBase = dayBaseForParisDateStr(activeDate);
...
export async function getStopDepartures(
  ...
  const rtByTrip = new Map<string, TripDelay>();
  for (const f of feed) rtByTrip.set(f.tripId, f);
  const { todayStr, nowSec, dayBase } = parisNow();
  // Resolve all stop ids to consider: if the user clicked a parent station,
  // also include all of its children. If they clicked a child, include sibling
  // children of the same parent so both directions/platforms are listed.
  const target = gtfs.stops.get(stopId);
  const stopIds = new Set<string>([stopId]);
  if (target) {
    const parentId = target.locationType === 1 ? target.id : target.parentId;
    if (parentId) {
      for (const s of gtfs.stops.values()) {
        if (s.parentId === parentId || s.id === parentId) stopIds.add(s.id);
      }
    }
  }
  ...
  for (const [tripId, sched] of gtfs.stopTimes) {
    ...
    if (!st) continue;
    if (!fullDay && st.departure < nowSec - 60) continue;
    const route = gtfs.routes.get(trip.routeId);
    if (!route) continue;
    const rt = rtByTrip.get(tripId);
    let delay = 0;
  ...
  return fullDay ? out : out.slice(0, limit);
}
