...
import type { GtfsData } from "../lib/gtfs";

/**
 * Resolve an Endpoint to a single (lat,lng) for upstream APIs that need
 * coordinates instead of a stop ID. Returns null if the endpoint is a stop
 * group whose stops have no known coordinates.
 */
function endpointToCoord(ep: Endpoint, g: GtfsData): LatLng | null {
  if (ep.kind === "address") return { lat: ep.lat, lng: ep.lng };
  for (const id of ep.stopIds) {
    const s = g.stops.get(id);
...
router.get("/itinerary", async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (atRaw) {
      const n = Number(atRaw);
      if (Number.isFinite(n) && n > 0) atTime = Math.floor(n);
    }
    ...
    if (provider === "google" && hasGoogle) {
      // Use Google Directions when both endpoints have coordinates we can pass
      // through. For stop endpoints, look up the first stop's coordinates.
      const fromCoord = endpointToCoord(fromEp, g);
      const toCoord = endpointToCoord(toEp, g);
      if (fromCoord && toCoord) {
        try {
          const options = await googleDirectionsTransit(
            fromCoord,
            toCoord,
            atTime,
            6,
          );
          res.json({ options, provider: "google" });
          return;
        } catch (err) {
          // Fall through to local planner on Google failure.
          console.warn("Google Directions failed, falling back:", err);
        }
      }
    }
    const options = await planItineraryEndpoints(fromEp, toEp, 6, atTime);
    res.json({ options, provider: "local" });
  } catch (e) {
    next(e);
  }
});
