  ...
  return (
    <div className="flex h-screen w-screen flex-col overflow-hidden">
        ...
        {TABS.map((t) => {
          ...
          return (
            ...
            >
              ...
              {t.label}
              {showBadge && (
                <span className="absolute right-[20%] top-1 inline-flex h-4 min-w-4 items-center justify-center rounded-full bg-destructive px-1 text-[10px] font-bold text-destructive-foreground">
                  {alertCount}
                </span>
              )}
            </button>
      ...
      <Sheet open={mobileSheet} onOpenChange={setMobileSheet}>
        <SheetContent
          side="bottom"
          className="md:hidden h-[80vh] p-0 flex flex-col"
        >
          ...
          <PanelTabs tab={tab} setTab={setTab} alertCount={alertCount} />
          <div className="flex-1 min-h-0">
            <PanelBody
              tab={tab}
              loading={!staticQ.data}
...
function PanelBody({
  ...
  if (tab === "nearby") {
    return <Nearby onSelectStop={onPickStopById} />;
  }
  return <Alerts />;
}
