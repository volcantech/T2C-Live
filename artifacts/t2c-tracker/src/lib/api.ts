...
export const api = {
  ...
  departures: (stopId: string) =>
    get<{ departures: Departure[] }>(
      `/gtfs/stop/${encodeURIComponent(stopId)}/departures`,
    ),
  nearbyStops: (lat: number, lng: number) =>
    get<{ stops: NearbyStop[] }>(
      `/gtfs/nearby-stops?lat=${lat}&lng=${lng}&limit=8`,
    ),
  itinerary: (from: string, to: string) =>
    get<{ options: ItineraryOption[] }>(
      `/gtfs/itinerary?from=${encodeURIComponent(from)}&to=${encodeURIComponent(to)}`,
    ),
